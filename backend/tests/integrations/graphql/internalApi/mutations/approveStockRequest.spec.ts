...
const postStockRequestApprovalAlert = vi.spyOn(
  discordModule,
...
describe('approveStockRequest', () => {
  ...
  it('returns deleted and send notification', async () => {
    ...
    const approveStockRequest = result.data['approveStockRequest'];
    expect(approveStockRequest.deleted).toEqual(true);
    expect(postStockRequestApprovalAlert).toHaveBeenCalled();
  });
});
